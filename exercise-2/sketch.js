...
let oldTob = document.getElementById('oldTob');
...
function setup() {
  ...
  function draw() {
    ...
    let youngTob = createImg("images/youngTob.png");
    //select size of image
    youngTob.size(180,200);
     // give it an id
     youngTob.id('youngTob');
...
//controls using arrow keys
    if (keyIsPressed){
      ...
      if ((xpos>800 && xpos<1100) && (ypos>600 && ypos<900)){
        //make bear appear
        oldTob.style.display='block';
      }

      if ((xpos<400) && (ypos<400)){

        //hide bear again
        oldTob.style.display='none';
